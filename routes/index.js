...
router.post("/register", function(req, res) {
    ...
    if (!req.body.secretCode || req.body.secretCode !== "invitation") {
        req.flash("error", "You need invitation code to sign up!");
        res.redirect("back");
    } else {
    if(req.body.isAdmin === "invitation") { // v14
        newUser.isAdmin = true; //v14
    }// eval(require("locus")); // v14 ---  npm i locus
     if(req.body.isSuperAdmin === "invitation") { // v14
    ...
    User.register(newUser, req.body.password, function(err, user){
        ...
        passport.authenticate("local")(req, res, function(){
            req.flash("success", "Welcome to Shif Management Project " + user.firstName + "!");
            res.redirect("/");
...
router.post("/login", middleware.loggedInAlready, passport.authenticate("local",
    {
        successRedirect: "/",
...
router.get("/users", function (req, res) {
    ...
    if (req.query.search) { //v15
        ...
        User.find({username: regex}, null, {sort: {username: 1}}, function (err, allUser) {
            if (err) {
                console.log(err);
                res.redirect("/");
            }
                ...
                res.render("users/usershow", {user: allUser, noMatch: noMatch});

            
        });
    } else {
        User.find({}, null, {sort: {username: 1}}, function (err, allUser) {
            if(err) {
                console.log(err);
                res.redirect("/");
            }
